...
**In this tap**: nothing checks the cask itself. `Casks/catalog.rb` is
generated by GoReleaser during the tool's release, and that generated
output does not satisfy `brew style` — Homebrew's linter for casks
submitted to the official homebrew-cask repo, where files are
hand-written. The offenses it raises (a blank line before the closing
`end`, array indentation in stanzas like `zap`) are inherent to
GoReleaser's template, not mistakes we could fix without hand-editing a
generated file. GoReleaser-published taps such as k9s and GoReleaser's
own run no CI on their generated files for the same reason.
...
this before it reaches the tap.

## GitHub Actions workflows

This tap has one workflow in `.github/workflows/tests.yml`, running on
every push and pull request:

- `markdown` — runs Prettier and markdownlint across all `.md` files

If you ever want a real check on the cask, the right one is an install
test (`brew install westarete/tap/catalog && catalog --version`) on
Apple Silicon. That verifies the binary downloads, installs, and runs —
which is what matters — rather than its formatting.
